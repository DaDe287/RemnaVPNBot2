...
| `BROADCAST_BOT_TOKENS` | Дополнительные токены Telegram-ботов для рассылок из админки (через запятую, опционально) |
...
| `BROADCAST_BOT_TOKENS` | Токены ботов для рассылок через админку (через запятую). Если пусто, используется `BOT_TOKEN`; при заполнении рассылка пробует отправить сообщение каждым указанным ботом и игнорирует ошибки отдельных отправок. Чтобы основной бот тоже участвовал в такой рассылке, добавьте его токен в список. | `123:AAA,456:BBB` |
